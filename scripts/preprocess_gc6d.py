...
def build_best_per_contact_group(g_points, g_offsets, fric_coefs, collision, pose, obj_idx, cam_pose_inv=None):
    ...
    Args:
        ...
        pose:        (4, 4)            object-to-camera (or object-to-world) transform
    ...
    final_offsets = g_offsets[np.arange(Np), best_view, best_angle, best_depth, :]    # (Np, 3): angle, depth, 
...
class GC6DLoader(GraspClutter6D):
    def __init__(self, root, camera='realsense', split='train'):
        super(GC6DLoader, self).__init__(root, camera=camera, split=split)

    def loadSceneObjectList(self, sceneId, annId, camera):
        """Load object list and their camera-frame poses from scene_gt.json."""
        img_id = self.annId2ImgId(annId, camera)

        with open(os.path.join(self.root, 'scenes', '%06d' % sceneId, 'scene_gt.json')) as f:
            scene_gt = json.load(f)

        obj_list  = []
        pose_list = []
        for obj in scene_gt[str(img_id)]:
            pose = np.eye(4)
...
def load_scene_grasp_labels(g, scene_id, camera):
    ...
    obj_list, _ = g.loadSceneObjectList(scene_id, 0, camera)
...
def main(cfgs):
    ...
    g = GC6DLoader(cfgs.dataset_root, camera=cfgs.camera, split=cfgs.split)
    ...
    for scene_id in tqdm(scene_ids):
        ...
        for ann_id in range(13):
            print('processing scene %06d img %06d' % (scene_id, g.annId2ImgId(ann_id, cfgs.camera)))
...
def process_scene(cfgs, g, scene_id, ann_id, grasp_labels, collision_labels):
    ...
    output_path = os.path.join(
        output_root, split,
        '%06d_%06d' % (scene_id, g.annId2ImgId(ann_id, camera))
    ) + '.h5'
    if os.path.exists(output_path):
        return

    # Load object list and poses for this ann_id (already in camera frame: cam_R_m2c, cam_t_m2c)
    obj_list, pose_list = g.loadSceneObjectList(scene_id, ann_id, camera)

    collision_dump = collision_labels[str(scene_id).zfill(6)]

    scene_clouds    = g.loadScenePointCloud(scene_id, camera=camera, annId=ann_id, margin=outlier)
    ...
    for i, (obj_idx, pose) in enumerate(zip(obj_list, pose_list)):
        ...
        # pose is already in camera frame, so cam_pose_inv=None
        grasp_group = build_best_per_contact_group(
            g_points, g_offsets, fric_coefs, collision, pose, obj_idx, cam_pose_inv=None
        )
    ...
    if not os.path.exists(os.path.join(output_root, split)):
        os.makedirs(os.path.join(output_root, split))
...
def visualize_result(scene_cloud, contact_points, grasp_rot, grasp_trans, grasp_width, scene_id, ann_id):
    ...
    o3d.visualization.draw_geometries(
        geoms,
        window_name='scene %06d ann %04d  |  %d contact points  |  %d grasps shown' % (
            scene_id, ann_id, M, K)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_root',
                        default=os.environ.get('GC6D_ROOT'),
                        help='Dataset root for GraspClutter6D (default: $GC6D_ROOT)')
    parser.add_argument('--camera', default='realsense-d435',
                        choices=('realsense-d415', 'realsense-d435', 'azure-kinect', 'zivid'),
                        help='realsense-d415, realsense-d435, azure-kinect, zivid')
