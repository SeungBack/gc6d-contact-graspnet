...
# from data.base_dataset import NoPositiveGraspsException

def collate_fn(batch):
    # check if the batch has None
    batch = [x for x in batch if x is not None]
    pc = torch.tensor([batch[i]['pc'] for i in range(len(batch))])
    collision_pc = torch.tensor([batch[i]['collision_pc'] for i in range(len(batch))])
    grasp_rt = torch.tensor([batch[i]['grasp_rt'] for i in range(len(batch))])
    quality = torch.tensor([batch[i]['quality'] for i in range(len(batch))])
    target_cps = torch.tensor([batch[i]['target_cps'] for i in range(len(batch))])
    target_cps_sym = torch.tensor([batch[i]['target_cps_sym'] for i in range(len(batch))])
    mesh = [batch[i]['mesh'] for i in range(len(batch))]
    return {'pc': pc, 'collision_pc': collision_pc, 'mesh': mesh, 'grasp_rt': grasp_rt, 
            'quality': quality, 'target_cps': target_cps, 'target_cps_sym': target_cps_sym}
