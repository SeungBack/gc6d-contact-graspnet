...
class ContactGraspNet(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        arg_configs = getattr(config, 'arg_configs', []) or []
        self.cfg = load_config('model_params.yaml', arg_configs=arg_configs)
        ...
        self.input_xyz_as_features = self.model_cfg.get('input_xyz_as_features', False)

        sa1_in_channel = 3 if self.input_xyz_as_features else 0
        ...
        self.bin_vals = torch.tensor(bin_vals, dtype=torch.float32)
    ...
    def forward(self, pc):
        ...
        if self.model_cfg['raw_num_points'] != self.model_cfg['ndataset_points']:
            if pc.shape[1] > self.model_cfg['ndataset_points']:
                inds = np.random.choice(pc.shape[1], self.model_cfg['ndataset_points'], replace=False)
            else:   
                inds = np.random.choice(pc.shape[1], self.model_cfg['ndataset_points'], replace=True)
            pc = pc[:, inds, :]

        pc = pc.permute(0, 2, 1).contiguous() # (B, N, 3) -> (B, 3, N)
        l0_xyz = pc[:, :3, :]
        l0_points = l0_xyz if self.input_xyz_as_features else None
        ...
        width_bin_pred_vals = self.bin_vals.to(argmax_indices.device)[argmax_indices]
    ...
    def build_6d_grasp(self, grasp_app, grasp_dir, contact_pts, thickness, use_torch=False, base_depth=0.02):
        ...
        if use_torch:
            ...
            ones = torch.ones((contact_pts.shape[0], contact_pts.shape[1], 1, 1), dtype=torch.float32).cuda()
            zeros = torch.zeros((contact_pts.shape[0], contact_pts.shape[1], 1, 3), dtype=torch.float32).cuda()
